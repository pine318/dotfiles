...
if [[ "$(uname -r)" != *microsoft* ]] && command -v ssh-agent 1>/dev/null 2>&1; then
  # ssh-agent プロセスが存在しないならば、これを起動しつつ、設定をファイルに出力
  if [ -d $SSH_DIR ] && [ $( ps -ef | grep ssh-agent | grep -v grep | wc -l ) -eq 0 ]; then
    ssh-agent >| $SSH_AGENT_FILE
    # SSH経由のログインでないならば，設定ファイルを読み込み，SSH_DIR 配下にある id_rsa, id_ecdsa という名前のファイルを ssh-add
    if [ ! -n "$SSH_CONNECTION" ]; then
      source $SSH_AGENT_FILE >& /dev/null
      find $SSH_DIR -name id_rsa -or -name id_ecdsa | xargs ssh-add >& /dev/null
    fi
  fi
